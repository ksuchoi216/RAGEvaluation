...
API 키는 코드에 넣지 않고 `OPENAI_API_KEY`, `ANTHROPIC_API_KEY` 환경 변수로
설정합니다. MLflow 저장 위치는 필요하면 `MLFLOW_TRACKING_URI`로 지정합니다.
지정하지 않으면 MLflow 기본 저장소를 사용합니다.

## 사용 예시

사용 가능한 모델명을 명시적으로 전달합니다. 아래 환경 변수 이름은 예시이며,
모델명을 직접 문자열로 전달해도 됩니다.
...
evaluator = RAGEvaluator.from_models(
    openai_model=os.environ["EVAL_OPENAI_MODEL"],
    anthropic_model=os.environ["EVAL_ANTHROPIC_MODEL"],
    mlflow_model=f"openai:/{os.environ['EVAL_OPENAI_MODEL']}",
)
...
- `evaluator.py`: 입력 검증, 평가 실행, 결과와 다수결
