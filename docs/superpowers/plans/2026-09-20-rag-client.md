...
- [ ] 가짜 검색 저장소를 이용해 단계 override, 빈 결과 formatting, 후보 수 검증, 예외 전파 테스트를 작성하고 실행한다.
- [ ] 공통 파이프라인과 단일 dense 검색 연결을 구현한다.
- [ ] `python -m pytest -q ailib/tests/test_rag_client.py`로 검증한다.
...
- [ ] HyDE의 dense/sparse 입력 분리, RRF 순위, stable chunk ID 중복 제거, 결합 점수 보존, 잘못된 가중치 테스트를 먼저 실행한다.
- [ ] 경로 내부 복수 쿼리 결합 후 경로 간 weighted RRF를 구현한다. 동일 순위는 입력 순서를 유지한다.
- [ ] 독립적으로 계산한 기대값으로 검증한다.
...
- [ ] expansion, 원래 질문 기반 rerank, 검색 전후 점수 보존, threshold 방향, 빈 입력, malformed 모델 출력, backend 점수 계약 테스트를 먼저 실행한다.
- [ ] 기존 기능을 concrete class로 옮기고 backend score metadata를 제공한다. 준비된 실제 FAISS로 integration 테스트할 수 있으면 수행한다.
- [ ] 사용 예시와 migration 안내를 작성한다.
- [ ] 타깃 pytest, 상위 프로젝트 pytest, ailib/tests 및 변경 파일 lint를 실행한다.
- [ ] diff를 검토하고 결과와 한계를 기록한다.
